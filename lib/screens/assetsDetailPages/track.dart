...
class Trackpage extends StatefulWidget {
  ...
  // final Function(int) onNavigateToMaintenanceForTrack;
  // final Function(int) onNavigateToAlertsForTrack;
  const Trackpage({
    super.key,
    required this.onCardsChange,
    required this.selectedStationName,
    // required this.maintenanceKey,
    // required this.onNavigateToMaintenanceForTrack,
    // required this.onNavigateToAlertsForTrack,
  });
...
class _TrackpageState extends State<Trackpage> {
  ...
  Map<String, List<SalesData>> socketTrackData = {};
  IO.Socket? socket;
...
int chartSeconds=1;
    ...
    try {
      ...
      setState(() {
        ...
        if (filterGroupName.isNotEmpty) {
          ...
          getTrackMaintenanceData(filterGroupName.first, 0, 0);
        }
      });
  ...
  Future<void> getTrackMaintenanceData(
      dynamic data, int fetchId, int pageIndex) async {
    if (data == null || data is! Map || data["groupname"] == null) return;
    final String url="$getAllAlertsMaintenance?station=${widget.selectedStationName}&assets=Track&group_name=${data["groupname"] ?? ""}";
    try {
      var res = await dioInstance.get(url
);
  ...
  Future<void> fetchForPage(int pageIndex) async {
    ...
    await getTrackMaintenanceData(data, fetchId, pageIndex);
  ...
  Future<void> onRefresh() async {
    ...
    await getTrackData();
    if (filterGroupName.isNotEmpty) {
      await getTrackMaintenanceData(filterGroupName[analyticsIndex], 0, 0);
    }
            ...
            Expanded(
              ...
              child: Column(
                ...
                children: [
                  ...
                  buildHeader(sortedData, groupData['groupname']),
                ...
                children: [
                  ...
                  Expanded(
                    child: SingleChildScrollView(
                      ...
                      child: buildAccordions(rank1Track),
                    ),
                  ),
                ],
              ),
  ...
  Widget buildHeader(dynamic sortedData, dynamic groupName) {
    return Card(
      elevation: 1.r,
      child: InkWell(
        splashColor: Colors.transparent,
        ...
        child: Container(
          ...
          child: Row(
            children: [
              SizedBox(width: 10.w),
              SvgPicture.asset("assets/images/of-track.svg",
                  width: 50.w,
                  colorFilter:
            ...
            child: Row(
              ...
              children: [
                ...
                UiHelper.smallText_bold(
                  text: sortedData['value'].toStringAsFixed(2),
                ),
                SizedBox(width: 10.w),
              ],
            ),
...
if (apiMinTimeValue!.isAtSameMomentAs(apiMaxTimeValue!)) {
  ...
  List<LineSeries<SalesData, DateTime>> lineSeries(
    ...
    return tagKeys.map<LineSeries<SalesData, DateTime>>((tag) {
      ...
      List<SalesData> combinedData = [...apiDataList, ...?socketTrackData[tag]];
    combinedData.sort(
  ...
  List<Map<String, dynamic>> getRank1Track(
      List<dynamic> dataList, List<String> tagKeys) {
    List<Map<String, dynamic>> rank1Track = [];
    for (var item in dataList) {
      if (item is Map) {
        item.forEach((key, value) {
          if (tagKeys.contains(key)) {
            for (var track in value) {
              if (track['rank'] == "1") rank1Track.add(track);
            }
          }
        });
      }
  ...
  Widget buildAccordions(List<Map<String, dynamic>> rank1Track) {
    return AnimatedContainer(
      duration: const Duration(milliseconds: 300),
 ...
 mainAxisSize: MainAxisSize.min,
                ...
                SizedBox(
                  ...
                  child: Scrollbar(
                    child: ListView.separated(
                      ...
                      itemCount: rank1Track.length,
                      separatorBuilder: (_, __) => SizedBox(height: 5.h),
                      itemBuilder: (context, index) {
                        final track = rank1Track[index];
                        final sig = track['description']
                            .toString()
                            .replaceAll("Double", "")
                            .replaceAll(":", "")
                            .trim();
                        final backColor = sig == "Yellow"
                            ...
                            : sig == "Red"
                                ...
                                : sig == "Green"
                                    ...
                                    : Colors.transparent;
                        final isDouble =
                            track['tagid'].toString().contains("HHG");
                        return SizedBox(
                          height: 30.h,
                          child: Row(
                            children: [
                    ...
                    ? Column(
                        children: [
                          ...
                          SizedBox(
                            width: 340.w,
                            height: 210.h,
                            child: Text("alertsaccordians"),
                            
                            // TrackAlertsAccordians(
                            //   maintenanceList: maintenanceData is List
                            //       ? maintenanceData
                            //       : [maintenanceData],
                            //   onShowAlertsDetailsForTrack: () =>
                            //       widget.onNavigateToAlertsForTrack(2),
                            // ),
                          ),
                        ],
                      )
                    ...
                    ? Column(
                        children: [
                                  ...
                                  child: Card(
                                    ...
                                    child: Container(
                                      ...
                                      child: InkWell(
                                        ...
                                        //           maintenanceData);
                                        //   widget
                                        //       .onNavigateToMaintenanceForTrack(
                                        //           1);
                                        // },
                                        child: Center(
                                            child: UiHelper.customText(
                          ...
                          SizedBox(
                            ...
                            //  TrackMaintenanceAccordians(
                            //   maintenanceList: maintenanceData is List
                            //       ? maintenanceData
                            //       : (maintenanceData != null
                            //           ? [maintenanceData]
                            //           : []),
                            //   onShowMaintenanceDetailsForTrack: (detail) {
                            //     widget.maintenanceKey.currentState
                            //         ?.setMaintenanceDataFromSignal(detail);
                            //     widget.onNavigateToMaintenanceForTrack(2);
                            //   },
                            // ),
                          ),
                        ],
    ...
    if (isAnalyticsOpen) {
      ...
      await getTrackMaintenanceData(
        filterGroupName[analyticsIndex], 0, 0);      
                ...
                : (maintenanceData != null && maintenanceData.isNotEmpty)
                    ? SizedBox(
                        ...
                        child: Container(
                          ...
                          child: Text("historyaccordian"),
                          
                          
                          // TrackHistoryAccordians(
              ...
              children: [
                AnimatedBuilder(
                  ...
                  builder: (context, _) => Transform.translate(
                      offset: Offset(0, controller.value * 100.h),
                      child: child),
                ),
                AnimatedBuilder(
                  animation: controller,
                ...
                SizedBox(
                  ...
                  child: loader
                      ...
                      : (filterGroupName.isEmpty
                          ? Center(
                              child: Text("No Track Devices Found.",
                                  style: TextStyle(
                                      fontSize: 20.r,
                          ...
                          : PageView.builder(
                              ...
                              itemBuilder: (context, index) =>
                                  buildTrackCard(index, filterGroupName[index]),
                            )),
                )
              ],
            ),
